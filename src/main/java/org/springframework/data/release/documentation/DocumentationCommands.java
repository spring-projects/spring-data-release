...
public class DocumentationCommands extends TimedCommand {
	...
	public void checkLinks(@CliOption(key = "", mandatory = true) TrainIteration iteration) {

		iteration.getModulesExcept(BUILD, BOM, COMMONS).forEach(module -> {

			StaticResources resources = new StaticResources(module);
			checkLinks(resources.getDocumentationUrl());
		});
	}

	@CliCommand("docs check-links-url")
	public void checkLinks(@CliOption(key = "", mandatory = true) String url) {
		operations.checkDocumentation(url).prettyPrint();
	}
}
